...
gui_spawn:
    ...
    Gui, Add, Edit, %gui_control_options% vuser_command gFindus
    Gui, Show,, myGUI
...
gui_search_add_elements:
    ...
    GuiControl, Disable, user_command
    Gui, Show, AutoSize
    return
...
gui_commandlibrary:
    ...
    {
        ; search for the string If user_command =, but search for each word individually because spacing between words might not be consistent. (might be improved with regex)
        If Substr(A_LoopReadLine, 1, 1) != ";" ; Do not display commented commands
        {
            If A_LoopReadLine contains if
            {
                IfInString, A_LoopReadLine, user_command
